...
const accounts = ["MaYing", "https://www.maying.co/auth/login", "donghaiz99@gmail.com", "crazy900727"]
...
function dataResults(url, checkinMsg, title) {
    ...
    $httpClient.get(url.replace(/(auth|user)\/login(.php)*/g, "") + userPath, function (error, response, data) {
        var usedData = data.match(/(>*\s*已使用(里程|流量|\s\d.+?%|：))[^B]+/)
        if (usedData) {
            usedData = usedData[0].match(/\d\S*(K|G|M|T)/)
            var restData = data.match(/(>*\s*(剩余量)(里程|流量|\s\d.+?%|：))[^B]+/)
            restData = restData[0].match(/\d\S*(K|G|M|T)/)            
            var allData = data.match(/(>*\s*(总流量)(里程|流量|\s\d.+?%|：))[^B]+/)
            allData = allData[0].match(/\d\S*(K|G|M|T)/)
            $notification.post(title, checkinMsg, "🧩 套餐总量：" + allData[0] + "B" + "\n👾 已用流量：" + usedData[0] + "B" + "\n😑 剩余流量：" + restData[0] + "B");
        } else {
